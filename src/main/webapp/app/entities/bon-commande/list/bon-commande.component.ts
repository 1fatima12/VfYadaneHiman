...
export class BonCommandeComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.bonCommandeService
      ...
      .subscribe({
        next: (res: HttpResponse<IBonCommande[]>) => {
  ...
  trackId(_index: number, item: IBonCommande): number {
    return item.id!;
  }

  delete(bonCommande: IBonCommande): void {
    const modalRef = this.modalService.open(BonCommandeDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IBonCommande[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
