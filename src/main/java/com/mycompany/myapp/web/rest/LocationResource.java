...
public class LocationResource {
    ...
    public ResponseEntity<List<LocationDTO>> getAllLocations(
        ...
        if ("employe-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where employe is null");
            return new ResponseEntity<>(locationService.findAllWhereEmployeIsNull(), HttpStatus.OK);
        }

        if ("client-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where client is null");
            return new ResponseEntity<>(locationService.findAllWhereClientIsNull(), HttpStatus.OK);
        }

        if ("fournisseur-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where fournisseur is null");
            return new ResponseEntity<>(locationService.findAllWhereFournisseurIsNull(), HttpStatus.OK);
        }

        if ("magazin-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where magazin is null");
            return new ResponseEntity<>(locationService.findAllWhereMagazinIsNull(), HttpStatus.OK);
        }
